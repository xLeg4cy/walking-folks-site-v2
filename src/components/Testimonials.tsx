...
const getIndustryIcon = (industry: string) => {
  switch (industry.toLowerCase()) {
    case 'travel':
    case 'viajes':
      return <PremiumIcon icon={Plane} size="sm" />;
    case 'software':
      return <PremiumIcon icon={Code} size="sm" />;
    case 'supply chain':
    case 'cadena de suministro':
      return <PremiumIcon icon={Package} size="sm" />;
    default:
      return <PremiumIcon icon={Plane} size="sm" />;
  }
};
                    ...
                    <div className="p-0 h-full">
                      ...
                      >
                        <div className="flex items-center justify-between mb-6">
                          <div>
                            <h4 className="font-bold text-lg text-foreground dark:text-white group-hover:text-[#4338CA] transition-colors duration-300">
                              {testimonial.name}
                            </h4>
                            <p className="text-muted-foreground dark:text-gray-400 text-sm mt-1">{testimonial.role}</p>
                          </div>
                        </div>

                        <Separator className="mb-6 bg-gray-100 dark:bg-gray-700/50" />

                        <div className="relative flex-grow">
                          <p className="text-muted-foreground dark:text-gray-300 text-base relative z-10 leading-relaxed">
                            "{testimonial.content}"
                          </p>
                        </div>
                      </motion.div>
